...
// this file is deployed as impl-<timestamp>.js; the stamp keeps GType
// registration unique across re-imports within one shell session
const _IMPL_STAMP = (import.meta.url.match(/impl-(\d+)\.js$/) || [])[1] || 'src';

const AIIndicator = GObject.registerClass(
{
    GTypeName: `AIIndicator_${_IMPL_STAMP}`,
},
...
// instantiated by extension.js (the loader) with the Extension instance,
// which provides .path and .uuid
export default class AIUsageMonitorImpl {
    constructor(loader) {
        this._loader = loader;
    }

    enable() {
        this._indicator = new AIIndicator(this._loader);
        Main.panel.addToStatusArea(this._loader.uuid, this._indicator, 0, 'center');
    }
